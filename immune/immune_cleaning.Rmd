...
pcsr_immune$siga_frc <- pcsr_immune$siga_raw * pcsr_immune$fr # still need to change below vars to siga_frc
```
...
if (crp_summary$skew > 2 | crp_summary$skew < -2 | crp_summary$kurtosis > 2 | crp_summary$kurtosis < -2) {
  pcsr_immune$ln_crp <- as.numeric(lapply(pcsr_immune$crp_raw, log))
}
if (il1b_summary$skew > 2 | il1b_summary$skew < -2 | il1b_summary$kurtosis > 2 | il1b_summary$kurtosis < -2) {
  pcsr_immune$ln_il1b <- as.numeric(lapply(pcsr_immune$il1b_raw, log))
}
if (siga_summary$skew > 2 | siga_summary$skew < -2 | siga_summary$kurtosis > 2 | siga_summary$kurtosis < -2) {
  pcsr_immune$ln_siga <- as.numeric(lapply(pcsr_immune$siga_frc, log))
}
if (il6_summary$skew > 2 | il6_summary$skew < -2 | il6_summary$kurtosis > 2 | il6_summary$kurtosis < -2) {
  pcsr_immune$ln_il6 <- as.numeric(lapply(pcsr_immune$il6_raw, log))
}

# Check everything post transformation for any transformed variables 
final_crp_summary <- describe(pcsr_immune$crp_raw)
final_il1b_summary <- describe(pcsr_immune$ln_il1b)
final_siga_summary <- describe(pcsr_immune$ln_siga)
final_il6_summary <- describe(pcsr_immune$ln_il6)

hist(pcsr_immune$crp_raw)
hist(pcsr_immune$ln_il1b)
hist(pcsr_immune$ln_siga)
hist(pcsr_immune$ln_il6)

ggplot(pcsr_immune, aes(x = "", y = ln_il1b)) +   
  geom_boxplot() +
  ylab("log (IL1-B)") +
  ggtitle("PCSR IL1-B (log transformed)") +
  geom_smooth(method='lm', color="black")
ggplot(pcsr_immune, aes(x = "", y = ln_siga)) +   
  geom_boxplot() +
  ylab("log (SIgA)") +
  ggtitle("PCSR SIgA (log transformed)") +
  geom_smooth(method='lm', color="black")
ggplot(pcsr_immune, aes(x = "", y = ln_il6)) +   
  geom_boxplot() +
  ylab("log (IL-6)") +
  ggtitle("PCSR IL-6 (log transformed)") +
  geom_smooth(method='lm', color="black")
...
crp_num_right_outliers <- length(na.omit(pcsr_immune$crp_raw [pcsr_immune$crp_raw > crp_up_limit])) 
crp_num_right_outliers
crp_num_left_outliers <- length(na.omit(pcsr_immune$crp_raw [pcsr_immune$crp_raw < crp_lo_limit])) 
crp_num_left_outliers 
il1b_num_right_outliers <- length(na.omit(pcsr_immune$ln_il1b [pcsr_immune$ln_il1b > il1b_up_limit]))
il1b_num_right_outliers
il1b_num_left_outliers <- length(na.omit(pcsr_immune$ln_il1b [pcsr_immune$ln_il1b < il1b_lo_limit])) 
il1b_num_left_outliers
siga_num_right_outliers <- length(na.omit(pcsr_immune$ln_siga [pcsr_immune$ln_siga > siga_up_limit]))
siga_num_right_outliers 
siga_num_left_outliers <- length(na.omit(pcsr_immune$ln_siga [pcsr_immune$ln_siga < siga_lo_limit])) 
siga_num_left_outliers
il6_num_right_outliers <- length(na.omit(pcsr_immune$ln_il6 [pcsr_immune$ln_il6 > il6_up_limit])) 
il6_num_right_outliers
il6_num_left_outliers <- length(na.omit(pcsr_immune$ln_il6 [pcsr_immune$ln_il6 < il6_lo_limit])) 
il6_num_left_outliers

```
In this data, there is one SIgA left outlier, and one IL-6 right outlier only. Winsorize these values.
```{r}
library(DescTools)
pcsr_immune$siga_w <- Winsorize(pcsr_immune$ln_siga, minval = siga_lo_limit, maxval = siga_up_limit, na.rm = TRUE)
pcsr_immune$il6_w <- Winsorize(pcsr_immune$ln_il6, minval = il6_lo_limit, maxval = il6_up_limit, na.rm = TRUE)
```


Output final cleaned data
```{r}
pcsr_immune$crp_cleaned <- pcsr_immune$crp_raw
pcsr_immune$il1b_cleaned <- pcsr_immune$ln_il1b
pcsr_immune$siga_cleaned <- pcsr_immune$siga_w
pcsr_immune$il6_cleaned <- pcsr_immune$il6_w
