...
```{r}
vol_ml = 2.5
pcsr_immune$fr <- vol_ml / pcsr_immune$flowsec
pcsr_immune$siga_frc <- pcsr_immune$siga * pcsr_immune$fr # still need to change below vars to siga_frc
```
...
ggplot(pcsr_immune, aes(x = "", y = il1b_raw)) +   
  ...
  ggtitle("PCSR IL1-B") +
  geom_smooth(method='lm', color="black")
ggplot(pcsr_immune, aes(x = "", y = siga_raw)) +   
  geom_boxplot() +
  ylab("SIgA (ng/ml)") +
  ggtitle("PCSR SIgA") +
  geom_smooth(method='lm', color="black")
...
il1b_summary
siga_summary <- describe(pcsr_immune$siga_raw)
siga_summary
...
if (siga_summary$skew > 2 | siga_summary$skew < -2 | siga_summary$kurtosis > 2 | siga_summary$kurtosis < -2) {
  pcsr_immune$ln_siga <- as.numeric(lapply(pcsr_immune$siga_raw, log))
}
...
describe(pcsr_immune$ln_il1b)
describe(pcsr_immune$ln_siga)
describe(pcsr_immune$ln_il6)
